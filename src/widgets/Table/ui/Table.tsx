...
const Table: React.FC<TableProps> = ({theadObj, teacher,}) => {
    const [open, setOpen] = useState(false);
    const [selectedItemId, setSelectedItemId] = useState<Person[] | null>(null);
    const [assentModalOpen, setAssentModalOpen] = useState<boolean>();
    const [selectedItem, setSelectedItem] = useState<string | null>(null)
    const [confirm, setConfirm] = useState<string | null>(null)
    const [openEdit, setOpenEdit] = useState<boolean>(false)

    const openModal = useAppSelector((state:RootState) => state.employees.assentModal);
    ...
    const submitGreen = (e: React.FormEvent) => {
        ...
        dispatch(setTableDataStudentCursach(tableData.filter(item => item.id !== id)))
        dispatch(studentCursachDelete({id}))
    ...
    return (
        <>
            ...
            {
                openEdit && <StudentCursachModalCreate titlePage={"Редактировать дипломную работу"} setOpen={setOpenEdit} selectedItem={selectedItemId}/>
            }
